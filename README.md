...
1. Access the API:
1. Access the API:
   - `http://localhost:8080/route?json={"locations":[{"lat":40.7484,"lon":-73.9857},{"lat":40.7306,"lon":-73.9352}],"costing":"auto"}`
   - `http://localhost:8080/route?json={"locations":[{"lat":40.7484,"lon":-73.9857},{"lat":40.7306,"lon":-73.9352}],"costing":"auto"}`
1. Stop the service:
1. Stop the service:
   ````sh
...
- `WORKERS`: default 1
- `PORT`: Railway usually sets; defaults to 8080 if absent
